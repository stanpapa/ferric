use crate::tools::math::scalar::ScalarNew;
...
// pub struct MatrixSym<T: Scalar>(BaseMatrix<T>);
...
#[derive(Clone)]
pub struct Matrix<T: ScalarNew, R: Dox, S> {
    elem: Vec<T>,
}
...
impl<T: Scalar> BaseMatrix<T> {
    ...
    pub fn zero(rows: usize, cols: usize) -> BaseMatrix<T> {
