...
impl BasisShell {
  ...
  pub fn dim(&self) -> i32 { return 2 * self.shell.l + 1 }
  pub fn cdim(&self) -> i32 { return (self.shell.l + 1) * (self.shell.l + 2) / 2 }
...
impl CartesianBasisFunction {
  ...
  // fn normalize(mut self) {
    ...
    for a in self.exps.iter() {
      let Fijk: f64 = (factorial2(2*i-1) * factorial2(2*j-1) * factorial2(2*k-1)).into();
      self.norm.push((2.0_f64.powf(2.0*l+1.5) * a.powf(l+1.5) /
      (PI.powf(1.5) * Fijk)).sqrt());
    } // a
    ...
    for bra in 0..self.exps.len() {
      for ket in 0..self.exps.len() {
        N += self.norm[bra] * self.norm[ket] * self.coefs[bra] * self.coefs[ket] /
             1.0;
      } // ket
    } // bra
...
fn gaussian_components(l: &i32) -> Vec<[i32; 3]> {
  ...
  let mut i = *l; let mut j = 0; let mut k = 0;
  let mut count = 0;
  while {
